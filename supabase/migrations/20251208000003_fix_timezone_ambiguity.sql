-- Drop both potential conflicting signatures to ensure clean slate
DROP FUNCTION IF EXISTS public.request_timezone_change(uuid, character varying, integer[]);
DROP FUNCTION IF EXISTS public.request_timezone_change(uuid, text, integer[]);

-- Recreate the function with a single, clear signature using TEXT
-- This resolves the "Could not choose the best candidate function" error
CREATE OR REPLACE FUNCTION public.request_timezone_change(p_user_id uuid, p_new_timezone text, p_new_weekend_days integer[] DEFAULT NULL::integer[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- Validate: IANA format (basic regex check)
  IF p_new_timezone !~ '^[A-Za-z_]+/[A-Za-z_]+$' AND p_new_timezone != 'UTC' THEN
    RETURN jsonb_build_object(
      'status', 'rejected',
      'message', 'Invalid timezone format. Must be IANA format (e.g., America/New_York)'
    );
  END IF;

  -- Validate: weekend days (must be length 2, values 0-6)
  IF array_length(p_new_weekend_days, 1) != 2 THEN
    RETURN jsonb_build_object(
      'status', 'rejected',
