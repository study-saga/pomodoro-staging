-- Migration: Update handle_new_user() to Set Valid background_id
-- Description: Ensures new users get a valid background_id ('room-video') when signing up,
--              preventing the blank white background issue.
...
BEGIN
  ...
  INSERT INTO public.users (
    ...
    username,
    avatar,
    background_id,  -- ADDED: Explicitly set valid background
    created_at,
    updated_at
  ) VALUES (
    NEW.id,
    ...
    v_username,
    v_avatar,
    'room-video',  -- ADDED: Set to valid default (desktop default, app will adjust for mobile)
    NOW(),
    NOW()
  )
